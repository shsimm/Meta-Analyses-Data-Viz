...
reported_race_raw = DE_mega_race %>% 
  dplyr::filter(abs(logFC) > 0.25 & adj.P.Val < 0.05) %>% 
dplyr::filter(!is.na(hgnc_symbol) & hgnc_symbol != "")
reported_race_clean = c(reported_race_raw$hgnc_symbol)

# Create a vector of the CTRA gene set
...
CTRA_reported_race = list(A = reported_race_clean,
                 B = CTRA)

category_names <- c("Gene Expression Siganature of Reported Race",
                    "Controlled Transcriptional Response to Adversity (CTRA)")


twoway_venn <- draw_venn(CTRA_reported_race, category_names, 2)
twoway_venn

```

```{r Shared_CTRA_Reported_Race}
...
category_names <- c("Up in Black-Reported Samples",
                    "Up in White-Reported Samples",
