...
# WE CANNOT USE THE MODERATED F STATISTIC SO BREAK THE COEFS UP!!
race_NIMH <- DEG_NIMH_race %>% dplyr::rename_at("t",~"NIMH_t")
race_PPM <- DEG_PPM_race %>% dplyr::rename_at("t",~"PPM_t")

# Join the tables together
cmc_race_cohort <- dplyr::inner_join(race_NIMH, race_PPM, by = "ensembl_gene_id")
# Run the correlation test - be wary of this (reviewer)
corr_cmc_race_cohort <- cor.test(cmc_race_cohort$NIMH_t, cmc_race_cohort$PPM_t, method = "spearman")
# See the results
...
# This only shows the corr 
# stat_cor(method = "spearman", aes(label = ..r.label..), color = 'red')

#corr_race_cohort_named <- corr_race_cohort + 
 # geom_text_repel(data=cmc_race_cohort,
...
######################## Mega - Analysis
```{r Get the Mega df -- from code}

DE_mega_race = DE_mega
sig_race = DE_mega %>% dplyr::filter(abs(logFC) > 0.25 & adj.P.Val < 0.05)
#write.csv(DE_mega_race, "/Users/shuaybsimmons/Desktop/School/UofT MSc Medical Science/Research/Thesis Research/Data Frames/DEG\\DE_mega_race.csv",row.names=TRUE)

```

```{r Volcano of Mega-Analysis}

volcano_mega_race <- make_volcano_plot_race(DE_mega_race, x_lim = c(-4, 4), y_lim = c(0, 25))
volcano_mega_race
...
ggsave("volcano_mega_race_ctra.png", path = "/Users/shuaybsimmons/Desktop/School/UofT MSc Medical Science/Research/Thesis Research/Plots/Race/Mega-Analysis")
...
######################## DEG list check
```{r DEG list dimension check -- update when I have time}

sig_results = gene_names_race_meta %>% dplyr::filter(P.Value <0.05)
sig_results_adj = gene_names_race_meta %>% dplyr::filter(p.adj<0.05)
DEG_NIMH_race_sig = DEG_NIMH_race %>% dplyr::filter(adj.P.Val<0.05)
DEG_PPM_race_sig = DEG_PPM_race %>% dplyr::filter(adj.P.Val<0.05)
DEG_mega_sig = gene_names_race_mega %>% dplyr::filter(adj.P.Val<0.05)

# Compare the sig lists across NIMH, PPM, and the Meta-analysis
dim(DEG_NIMH_race_sig)
dim(DEG_PPM_race_sig)
dim(sig_results)
dim(sig_results_adj)
dim(DEG_mega_sig)
...
######################## Residualized Count Plots Analysis
...
```{r Plot EV.1 vs residualized Counts}
...
#ggsave("ancestry_vs_race.png", width = 6, height = 7, path = "/Users/shuaybsimmons/Desktop/School/UofT MSc Medical Science/Research/Thesis Research/Plots/Race/Mega-Analysis/")
...
# Race
  # Upregulated
  race = DE_mega_race %>% 
  dplyr::filter(adj.P.Val < 0.05) %>% dplyr::filter(!is.na(hgnc_symbol) & hgnc_symbol != "")
  race = c(race_upregulated$hgnc_symbol)
...
yo <- intersect(interferon, race_upregulated$hgnc_symbol) 
...
# Race
  ...
  dplyr::filter(logFC > 0.25 & adj.P.Val < 0.05) %>% dplyr::filter(!is.na(hgnc_symbol) & hgnc_symbol != "")
